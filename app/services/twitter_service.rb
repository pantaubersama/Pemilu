class TwitterService
  ...
  def create!
    @results.each do |tw|
      ...
      profile_picture              = URI.join("https://pbs.twimg.com", (url.path.split("/").reverse.drop(1).reverse + [(url.path.split("/").last.split("_").reverse.drop(1).reverse + ["200x200.jpg"]).join("_")]).join("/"))
